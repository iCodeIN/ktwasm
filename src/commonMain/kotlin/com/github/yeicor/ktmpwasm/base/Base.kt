...
sealed class WasmValue {
  @Suppress("unused")
  companion object {
    fun fromType(type: Type): WasmValue =
        when (type) {
    ...
    fun wrap(obj: Any): WasmValue =
        when (obj) {
          ...
          else -> throw Error("Cannot convert ${this::class.simpleName} to a WasmValue")
        }
  }
  ...
  fun toI32(): Int =
      when (this) {
        is I32Value -> value
        else -> throw Error("Cannot convert ${this::class.simpleName} to I32")
      }

  fun toI64(): Long =
      when (this) {
        is I64Value -> value
        else -> throw Error("Cannot convert ${this::class.simpleName} to I64")
      }

  fun toF32(): Float =
      when (this) {
        is F32Value -> value
        else -> throw Error("Cannot convert ${this::class.simpleName} to F32")
      }

  fun toF64(): Double =
      when (this) {
        is F64Value -> value
        else -> throw Error("Cannot convert ${this::class.simpleName} to F64")
      }

  fun assertI32(): WasmValue =
      when (this) {
        is I32Value -> this
        else -> throw Error("Value is ${this.type} not i32")
      }

  fun assertI64(): WasmValue =
      when (this) {
        is I64Value -> this
        else -> throw Error("Value is ${this.type} not i64")
      }

  fun assertF32(): WasmValue =
      when (this) {
        is F32Value -> this
        else -> throw Error("Value is ${this.type} not f32")
      }

  fun assertF64(): WasmValue =
      when (this) {
        is F64Value -> this
        else -> throw Error("Value is ${this.type} not f64")
      }

  fun assertVoid(): WasmValue =
      when (this) {
        is UnitValue -> this
