...
data class Module(
    ...
    fun refFunction(idx: Int): FunctionRef = when {
    ...
    override fun clone(): Module = Module(
        name = name,
        memory = memory.clone(),
        table = table.clone(),
        functions = functions,
...
data class BoundFunction(val module: Module, val function: Function) :
    FunctionRef {
...
data class Global(val mutable: Boolean = true, val type: Type, var value: WasmValue? = null) :
    GlobalRef {
    ...
    override fun set(value: WasmValue) {
        ...
        if (type != value.type) {
            throw Error("Type mismatch for global expected ${type}, got ${value.type}")
        }
        this.value = value
    }
