...
fun parseCodeSection(contents: ModuleContents, parser: ByteParser) {
    ...
    assert(contents.code == null)
    contents.code = parseVector(parser) {
        parseCode(parser, types)
    }
}
...
fun parseExpression(parser: ByteParser, types: List<FuncType>): List<Instruction> {
    ...
    Loop@ while (true) {
        val opcode = parser.readByte().toInt() and 0xFF
        ...
        val instr = when (opcode) {
            ...
            0x11 -> {
                ...
                assert(byte == 0.toByte())

                val type = types.getOrNull(typeidx) ?: throw Error("Cannot get type $typeidx")

                assert(type.result.size <= 1) { "Can only handle single return values" }
            ...
            0x3F -> {
                parseByte(parser).also { assert(it == 0.toByte()) }
                MemorySize()
            }
            0x40 -> {
                parseByte(parser).also { assert(it == 0.toByte()) }
            ...
            else -> throw Error("Unknown opcode ${Integer.toHexString(opcode)}")
        }
